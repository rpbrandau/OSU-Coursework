...
{
	/*write this*/
	if (cur == NULL) //if there isn't a node, build a node
	{
		struct Node *newNode = (struct Node *)malloc(sizeof(struct Node));
		assert(newNode != 0);
		//initialize pointers to left & right to null
		newNode->left = 0;
		newNode->right = 0;
		newNode->val = val;
		return newNode;
	}
	else if (compare(val, cur->val) == -1)  //if compare returns -1, r_side is greater, add a node to the left
	{
		cur->left = _addNode(cur->left, val);
	}
	else //if compare returns 1, l_side is greater, add a node to the right
	{
...
{
	/*write this*/
	struct Node *current = tree->root; //store the root as a starting point
	...
	while (current != NULL)
	{
		...
		else if (compare(val, current->val) == -1) //if compare returns R > L , the value entered is less than the current node (move left)
		{
			current = current->left;
		}
		else if (compare(val, current->val) == 1) //if compare returns L > R, the value entered is greater than the current node (move right)    
		{
...
TYPE _leftMost(struct Node *cur)
{
	...
	{
		return cur->val; //left most value
	}
	else
	{
		return _leftMost(cur->left); //if cur->left isnt null, perform the search again
	}
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	if (cur->left != NULL)
	{
		cur->left = _removeLeftMost(cur->left); //searching for most left node
	}
	else
	{
		if (cur->right != NULL)
...
{
	/*write this*/
	if (compare(val, cur->val) == 0) //if the node has the value being searched for
	{
		if (cur->right == NULL) //right is a dead end
		{
			return cur->left; //go left
		}
		else //remove node, re-arrange deleted node's children
		{
			cur->val = _leftMost(cur->right);
			cur->right = _removeLeftMost(cur->right);
		}
	}
	else if (compare(val, cur->val) == -1)  //if entered value is less than current node value
	{
